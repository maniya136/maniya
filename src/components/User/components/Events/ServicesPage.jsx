...
const VENDORS = {
  Catering: [
    ...
    },
    {
      ...
      image:
        "https://images.unsplash.com/photo-1544025162-1a07e2a1fdfa?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
  ],
  Photography: [
    {
    ...
    },
    {
      ...
      image:
        "https://images.unsplash.com/photo-1526170375885-4edd8f048b05?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
  ],
  Venue: [
    {
      ...
      image:
        "https://images.unsplash.com/photo-1541178370032-cb48160de2d4?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
    {
      id: 6,
      name: "Garden Oasis",
      ...
      image:
        "https://images.unsplash.com/photo-1519671482749-5f2cdb8f5a3b?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
  ],
  Decor: [
    {
  ...
  "Audio/Visual": [
    ...
    },
    {
      ...
      image:
        "https://images.unsplash.com/photo-1551818255-e6e10975bc17?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
  ],
};

const ServicesPage = ({ eventData = {}, onBack, onSubmit }) => {
  ...
  const handleContinue = () => {
    if (currentStep === 1) {
      ...
      // If onSubmit callback is provided, use it (for CreateEventSection)
      if (onSubmit) {
        onSubmit(finalData);
      } else {
        // Otherwise, navigate to confirmation page (for standalone use)
        navigate("/booking-confirmation", {
          state: { booking: { ...eventData, ...finalData } },
        });
      }
    }
  };
  ...
  const calculateTotalCost = () => {
    return Object.values(selectedVendors).reduce((total, vendor) => {
      if (!vendor || !vendor.price) return total;
  ...
  const areAllVendorsSelected = () => {
    return selectedServices.every(
      (serviceId) => selectedVendors[serviceId] && selectedVendors[serviceId].id
    );
  };
  ...
  const renderVendorCard = (service, vendor) => {
    ...
    return (
      <div
        ...
        onClick={() =>
          setSelectedVendors((prev) => ({ ...prev, [service.id]: vendor }))
        }
      >
            ...
            {Array(5)
              ...
              .map((_, i) => (
                <span
                  key={i}
                  className={`star ${
                    i < Math.floor(vendor.rating) ? "filled" : ""
                  }`}
                >
                  ★
                </span>
        ...
        <div className="page-header">
          ...
          <div className="header-content">
            ...
            <div className="event-summary">
              <h3>
                {eventData?.eventTitle || eventData?.eventName || "New Event"}
              </h3>
              <p>
                {eventData?.startDate || eventData?.eventDate || "Date TBD"} •{" "}
                {eventData?.city || eventData?.location || "Location TBD"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="progress-indicator">
          <div
            className={`progress-step ${
              currentStep === 1 ? "active" : currentStep >= 2 ? "completed" : ""
            }`}
          >
          ...
          <div className={`progress-step ${currentStep === 2 ? "active" : ""}`}>
            ...
            <div className="step-label">Choose Vendors</div>
          </div>
        </div>
          ...
          {currentStep === 1 ? (
            ...
            <div className="vendors-selection">
              <div className="section-header">
                ...
                <p className="subtitle">
                  Choose your preferred vendors for each service
                </p>
              </div>
              ...
              <div className="action-footer">
                ...
                <button
                  className={`continue-button ${
                    !areAllVendorsSelected() ? "disabled" : ""
                  }`}
                  onClick={handleContinue}
                  disabled={!areAllVendorsSelected() || isSubmitting}
                >
                  {isSubmitting
                    ? "Processing..."
